...
function autoSelectVNVoice() {
  const voiceSelect = document.getElementById("voiceSelect");
  const vnVoices = voices.filter(v => v.lang.includes("vi"));
  if (vnVoices.length > 0) {
    const index = voices.indexOf(vnVoices[0]);
    voiceSelect.selectedIndex = index;
    document.getElementById("log").innerText = "✅ Đã chọn giọng tiếng Việt: " + vnVoices[0].name;
  } else {
